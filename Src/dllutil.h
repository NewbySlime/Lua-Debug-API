...
namespace dynamic_library::util{
  // A helper class for calling a function at this object creation. This is helpful to workaround initiation for a thread using thread_local.
  class constructor_helper{
    public:
      typedef void (*constructor_cb)();

      constructor_helper(constructor_cb cb);
  };
