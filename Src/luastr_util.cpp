...
void _ts_table_iter_cb(const lua::api::core* lc, int key_idx, int value_idx, int iter_idx, void* cb_data){
  _ts_func_data* _fdata = (_ts_func_data*)cb_data;

  int _current_indent = _fdata->indent_offset+1;
  
  int _key_type = lc->context->api_value->type(lc->istate, key_idx);
  std::string _key_name = _to_string((lua_State*)lc->istate, key_idx, _fdata->_sr_data, 0);
  std::string _value_str = ""; 
  _value_str = _to_string((lua_State*)lc->istate, value_idx, _fdata->_sr_data, _current_indent);
